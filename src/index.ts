...
import { existsSync, writeFileSync, mkdirSync, appendFileSync, statSync, readdirSync, unlinkSync, watch, type FSWatcher } from 'fs'
 ...
 */
function notifyUser(context: any, status: 'success' | 'warning' | 'error', message: string, suggestion?: string): void {
  const icons = {
    success: '✅',
    warning: '⚠️',
    error: '❌',
 ...
 */
function setupConfigWatcher(context: any): void {
 ...
 */
function cleanupConfigWatcher(): void {
  if (configWatcher) {
    configWatcher.close()
    configWatcher = null
...
async function setupMCP(context: any): Promise<boolean> {
  ...
  if (!mcpProcess) {
    ...
    notifyUser(
      ...
      'Check logs at ~/.claude-pty/logs/mcp.log'
    )
  ...
  if (context.registerMCPServer) {
    try {
      ...
      notifyUser(
        context,
        'warning',
        'MCP server started but API registration failed',
  ...
  if (configOk) {
    notifyUser(
      context,
      'warning',
      'MCP server configured via config file',
      'Please restart Claude Code to activate'
    )
  } else {
    notifyUser(
      context,
      ...
      'Check file permissions for ~/.config/claude/'
    )
  }
