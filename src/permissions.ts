...
export function initPermissions(directory: string): void {
...
async function handleAskPermission(commandLine: string): Promise<never> {
  await denyWithToast(
    ...
    `PTY spawn denied: Command "${commandLine}" requires user permission which is not supported by this plugin. Configure explicit "allow" or "deny" in your permission settings.`
  )
...
export async function checkWorkdirPermission(workdir: string): Promise<void> {
  ...
  if (extDirPerm === 'deny') {
    await denyWithToast(
      `PTY spawn denied: Working directory "${workdir}" is outside project directory "${_projectDirectory}". External directory access is denied by user configuration.`
    )
  }
