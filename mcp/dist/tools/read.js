 ...
 */
function formatLine(line, lineNum, maxLength = MAX_LINE_LENGTH) {
 ...
 */
function handlePlainRead(session, offset, limit) {
    ...
    const formattedLines = result.lines.map((line, index) => formatLine(line, result.offset + index + 1));
 ...
 */
function handlePatternRead(session, pattern, ignoreCase, offset, limit) {
 ...
 */
export async function executePTYRead(args) {
