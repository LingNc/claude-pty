...
export function App() {
    ...
    const { connected: wsConnected, subscribeWithRetry, sendInput, } = useWebSocket({
        activeSession,
        onRawData: useCallback((rawData) => {
            setRawOutput((prev) => {
        ...
        onSessionList: useCallback((newSessions, autoSelected) => {
        ...
        onSessionUpdate: useCallback((updatedSession) => {
    ...
    const { handleSessionClick, handleSendInput, handleKillSession } = useSessionManager({
        ...
        sendInput,
        wsConnected,
        onRawOutputUpdate: useCallback((rawOutput) => {
            setRawOutput(rawOutput);
        }, []),
    });
    return (<div className="container" data-active-session={activeSession?.id}>
      <Sidebar sessions={sessions} activeSession={activeSession} onSessionClick={handleSessionClick} connected={connected}/>
      <div className="main">
        {activeSession ? (<>
            ...
            <div className="output-container">
              <RawTerminal key={activeSession?.id} rawOutput={rawOutput} onSendInput={handleSendInput} onInterrupt={handleKillSession} disabled={!activeSession || activeSession.status !== 'running'}/>
            </div>
